...
namespace AE.Net.Mail {
	...
	public class MailMessage : ObjectWHeaders {
		...
		public virtual void Load(string message) {
			bool headersOnly = false;
			this.Load(message, headersOnly);
		}
		public void Load(string message, bool headersOnly) {
			using (var mem = new System.IO.MemoryStream((Encoding ?? System.Text.Encoding.Default).GetBytes(message)))
				Load(mem, headersOnly, (int)mem.Length, null);
		}

		public virtual void Load(Stream reader, bool headersOnly) {
			this.Load(reader, headersOnly, 0, null);
		}

		public void Load(Stream reader, bool headersOnly, int maxLength, char? termChar) {
		...
		public virtual void Save(System.IO.Stream stream, Encoding encoding) {
