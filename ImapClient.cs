...
namespace AE.Net.Mail {
	...
	public class ImapClient : TextClient, IMailClient {
		...
		public virtual MailMessage[] GetMessages(string startUID, string endUID) {
			bool headersonly = true;
			bool setseen = false;
			return this.GetMessages(startUID, endUID, headersonly, setseen);
		}
		public virtual MailMessage[] GetMessages(string startUID, string endUID, bool headersonly, bool setseen) {
			return GetMessages(startUID, endUID, true, headersonly, setseen);
		}

		public virtual MailMessage[] GetMessages(int startIndex, int endIndex) {
			bool headersonly = true;
			bool setseen = false;
		...
		public virtual string[] Search(SearchCondition criteria) {
			bool uid = true;
		...
		public virtual Lazy<MailMessage>[] SearchMessages(SearchCondition criteria) {
			bool headersonly = false;
			return this.SearchMessages(criteria, headersonly);
		}
		public virtual Lazy<MailMessage>[] SearchMessages(SearchCondition criteria, bool headersonly) {
		...
		private string FlagsToFlagString(Flags flags) {
			return flags.ToString().Split(',').Select(x => "\\" + x.Trim()).Join(" ");
		}
